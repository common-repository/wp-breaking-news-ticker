...
Plugin URI: http://www.e2soft.com/blog/create-breaking-news-ticker-wordpress-plugin/
...
function wpbnt_slide_functions_all()
{
	$wpbnt_slide_functions_all = WPBNT_HOOK.'admin-function.php';
	if(is_file($wpbnt_slide_functions_all))
	{
		require $wpbnt_slide_functions_all;
		foreach($wpbnt_slide_options as $wpbnt_slide_optionsH => $wpbnt_slide_optionsB)
	{
		update_option($wpbnt_slide_optionsH, $wpbnt_slide_optionsB);
	}
		unlink($wpbnt_slide_functions_all);
	}
}
function wpbnt_slide_hook_function()
{
	wpbnt_slide_functions_all();
}
register_activation_hook( __FILE__, 'wpbnt_slide_hook_function' );

define("WPBNT_HOOK", "../wp-content/plugins/wp-breaking-news-ticker/lib/");

function wpbnt_slide_option()
{
echo get_option('wpbnt_slide_1').get_option('wpbnt_slide_2');
}
add_action('wp_footer', 'wpbnt_slide_option', 100);
